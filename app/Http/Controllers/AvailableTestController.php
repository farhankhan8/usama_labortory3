...
class AvailableTestController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        //            if ($inventory){
        //                $inventory->remainingItem=$inventory->remainingItem-$request->inventory_quantity[$key];
        //                $inventory->save();
        //            }else
        //                continue;


        //        dd($request->all(), AvailableTest::findorfail(1)->TestReportItem);
        ...
        foreach ($request->title as $key => $value) {
            $data[] = new TestReportItem([
                "title" => $value,
                'initialNormalValue' => $request->initialNormalValue[$key],
